...
namespace Gradual_Romance
{
    public class Thought_GRWantToSleepWithSpouseOrLover : Thought_Situational
    {
        public override string LabelCap
        {
            get
            {
                DirectPawnRelation directPawnRelation = RelationshipUtility.MostLikedBedSharingRelationship(this.pawn, false);
        ...
        protected override float BaseMoodOffset
        {
            get
            {
                float a = -0.05f * (float)this.pawn.relations.OpinionOf(RelationshipUtility.MostLikedBedSharingRelationship(this.pawn, false).otherPawn);
